...
if (isset($_POST['action'])) {
    switch($_POST['action']) {
        case 'create':
            addType($conn, $_POST['name'], $_POST['description'], $_POST['active'], $_POST['idBuilding'], $_POST['idTeachers'] );
            break;
        case 'update':
            updateType($conn, $_POST['id'], $_POST['name'], $_POST['description'], $_POST['active'], $_POST['idBuilding'], $_POST['idTeachers']);
            break;
        case 'delete':
...
function renderAddForm() {
    ...
    $result.= '<tr><td>idBuilding:</td><td><textarea name="idBuilding"></textarea></td></tr>';
    $result.= '<tr><td>idTeachers:</td><td><textarea name="idTeachers"></textarea></td></tr>';
...
function renderEditForm($conn, $id) {
    ...
    '</select></td></tr>';
    $result.= '<tr><td>idBuilding:</td><td><input type="text" name="idBuilding" required value="'.$item['idBuilding'].'"></td></tr>';
    $result.= '<tr><td>idTeachers:</td><td><input type="text" name="idTeachers" required value="'.$item['idTeachers'].'"></td></tr>';
